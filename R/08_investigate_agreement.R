...
ggsave("./output/manuscript_figs/project_agreement_heatmap.png",dpi=400,width = 8,height = 6)
...
unite %>% 
  ...
  bind_cols(data.frame(database="UNITE_Fungi"))
...
euk %>% 
  ...
  transform_sample_counts(function(x)x/sum(x)) %>% 
  estimate_richness(measures = "Shannon") %>% 
  bind_cols(data.frame(database="UNITE_All"))

u$study <- unite@sam_data$bio_project
e$study <- euk@sam_data$bio_project
...
full %>% 
  ggplot(aes(x=database,y=Shannon)) +
  ...
  labs(y="Shannon diversity",
       fill="Database")
ggsave("./output/manuscript_figs/Figure_3.png",dpi=400,height = 8,width = 10)

# compare beta-diversity changes, if any
u_mat <- 
unite %>% 
  subset_taxa(Kingdom == "k__Fungi") %>% 
  transform_sample_counts(function(x)x/sum(x)) %>% 
...
e_mat <- 
  ...
  subset_taxa(Kingdom == "k__Fungi") %>% 
  transform_sample_counts(function(x)x/sum(x)) %>% 
  subset_samples(sample_sums(euk %>% 
                             subset_taxa(Kingdom == "k__Fungi"))>0) %>% 
  otu_table() %>% 
  vegdist()
...
euk_beta <- 
betadisper(euk_sub@otu_table %>% vegdist(),
           group = euk_sub@sam_data$host)

unite_beta <- 
  betadisper(unite_sub@otu_table %>% vegdist(),
             group=unite_sub@sam_data$host)
