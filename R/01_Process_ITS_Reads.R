...
path <- "./Data/fastq/ITS2" # CHANGE to the directory containing your demultiplexed fastq files
filtpath <- file.path(path, "filtered") # Filtered files go into the filtered/ subdirectory
if(!file_test("-d", filtpath)) dir.create(filtpath) # make directory for filtered fqs if not already present
fns <- sort(list.files(file.path(path), full.names = TRUE, pattern = "_pass_1.fastq.gz.ITS2.gz"))


sample.names <- unlist(map(strsplit(basename(fns), "\\."), 1)) %>% str_split("_") %>% map_chr(1)
...
# IMPORT METADATA ####
meta = read_delim("./Data/PRJNA664779.csv",delim = ",")
row.names(meta) <- as.character(meta$Run)

# filter metadata to just rows that match remaining files
meta <- meta[meta$Run %in% row.names(seqtab.nochim),]
# reorder
meta <- meta[order(meta$Run),]

identical(meta$Run,row.names(seqtab.nochim))
...
row.names(met) <- meta$Run


ps <- phyloseq(otu,met,tax)

saveRDS(ps,"./output/ps_object_not_cleaned.RDS")
